  ...
  public boolean isCollision(GameObject object) {
    ...
    for (int i = 0; i < width; i++) {
      for (int j = 0; j < height; j++) {
        int objectX = i + (int) x - (int) object.x;
        int objectY = j + (int) y - (int) object.y;
        if (objectX < 0
            || objectX >= object.width
            || objectY < 0
            || objectY >= object.height) {
          continue;
        }
        if (matrix[j][i] != transparent
